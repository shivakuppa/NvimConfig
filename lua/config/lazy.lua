-- Shiva Kuppa's lazy.nvim initialization
	...
	spec = {
		-- Load LazyVim, but without any plugins (no import).
		{
			...
			config = function(_, opts)
				...
				local ShivaConfig = require('shiva.config')
				ShivaConfig.setup(user_setup.opts and user_setup.opts() or {})
				-- Setup lazyvim, but don't load lazyvim/config/* files.
				package.loaded['lazyvim.config.options'] = true
				opts = vim.tbl_deep_extend('force', ShivaConfig.opts(), opts)
